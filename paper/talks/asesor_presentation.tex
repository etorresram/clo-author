...
\begin{frame}{Resultado 1 -- TWFE continuo: el agregado parece nulo}
  ...
  \\
  En aislamiento, esto sugeriría una replicación del nulo de \citet{HumlumVestergaard2025_llm}.
  \\
  \textbf{Pero el cuadro cambia.}
\end{frame}
...
% --- Slide R4: Sintesis (Narrativa D refinada) ------------------------------
\begin{frame}{Síntesis -- Narrativa D refinada}
  \small
  El cuadro emergente para Lima Metropolitana:
  \vfill
  \begin{itemize}
    \setlength\itemsep{0.4em}
    \item \textbf{Sector formal} (alta $\beta$): \emph{expande} el empleo ($+57\%$ por unidad de $\beta$), \emph{reduce} las horas por trabajador ($-1.9$ a $-2.5$ hrs/sem), con presión bajista moderada en el salario hora ($-13\%$).
    \\
    Lectura: complementariedad sesgada por habilidad. Las firmas formales adoptan herramientas con LLM y crecen extensivamente, ajustan intensivamente.
    \item \textbf{Sector informal} (alta $\beta$): contracción débil del empleo, pero los sobrevivientes ganan $+27\%$ de salario hora.
    \\
    Lectura: selección positiva. Solo los informales más productivos permanecen en ocupaciones expuestas.
  \end{itemize}
  \vfill
  \normalsize
  \textbf{Esto es la \emph{Narrativa D refinada}} preregistrada: ganancias estratificadas con asimetría formal-informal. La preregistración de la formalidad como margen central queda \emph{empíricamente vindicada}.
\end{frame}
  ...
  \begin{itemize}
    \item \textbf{Completado para Perú.}
      \begin{itemize}
        ...
        \item Estimación principal: TWFE continuo, CS-2021 binario, event study Sun-Abraham (pre-trends OK), cotas Honest-DiD.
...
\begin{frame}{Preguntas para discutir}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Posicionamiento del paper.} ¿La Narrativa D refinada (selección informal positiva como hallazgo nuevo) justifica un nuevo canal teórico, o lo dejo como evidencia empírica sin modelar?
